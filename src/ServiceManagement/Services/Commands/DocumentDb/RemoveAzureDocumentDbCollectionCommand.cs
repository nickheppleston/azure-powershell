...
{
    /// <summary>
    /// Deletes a DocumentDb Document Collection
    /// </summary>
    [Cmdlet(VerbsCommon.Remove, "AzureDocumentDbCollection", SupportsShouldProcess = true), OutputType(typeof(bool))]
    public class RemoveAzureDocumentDbCollectionCommand : AzurePSCmdlet
    {
        ...
        [Parameter(Position = 5, HelpMessage = "Do not confirm the removal of the DocumentDb Collection")]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            else
            {
                try
                {
                    ConfirmAction(
                        Force.IsPresent,
                        String.Format(Resources.RemoveDocumentDbCollectionConfirmation, Id),
                        String.Format(Resources.RemovingDocumentDbCollectionMessage),
                        Id,
                        async () =>
                        {
                            try
                            {
                                await DocumentDbClient.DeleteDocumentCollectionsAsync(collection.SelfLink);
